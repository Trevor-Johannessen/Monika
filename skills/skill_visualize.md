...
All HTML files are stored in `/etc/monika/html`.

## Before creating a new file

1. Use `list_files` on `/etc/monika/html` to check for existing HTML files.
...
1. Use `write_file` to create the file in `/etc/monika/html`.
